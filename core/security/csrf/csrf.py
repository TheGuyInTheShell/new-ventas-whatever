...
class CSRFProvider:
    ...
    def validate_format(self, token: str) -> bool:
        ...
        expected = hmac.new(
            self.secret, salt.encode("utf-8"), hashlib.sha256
        ).hexdigest()
...
class CSRFExtension(Extension):
    ...
    def extends(self):
        ...
        self.app.add_middleware(cast(Any, CSRFMiddleware), provider=provider)
            ...
            async def async_inner(*args: P.args, **kwargs: P.kwargs) -> Any:
                ...
                for arg in args:
                    if hasattr(arg, "templates"):
                        template_provider = cast(
                            Jinja2Templates, getattr(arg, "templates")
                        )
                ...
                try:
                    if (
                        template_provider
                        and hasattr(template_provider, "env")
                        and request_obj
                    ):
