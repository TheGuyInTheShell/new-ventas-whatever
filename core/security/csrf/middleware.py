...
class CSRFMiddleware:
    ...
    def __init__(self, app: ASGIApp, provider: Any):
        self.app = app
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        async def send_wrapper(message: MutableMapping[str, Any]) -> None:
            if message["type"] == "http.response.start" and is_new:
                ...
                # Max-Age: 1 dia (86400)
                cookie_str = (
                    f"csrf_token={token}; HttpOnly; Path=/; Max-Age=86400; SameSite=Lax"
                )
        ...
        if method in ("POST", "PUT", "DELETE", "PATCH") and endpoint is not None:
            ...
            if config:
                ...
                if "body" in sources or "form" in sources and not is_valid:
                    ...
                    async def middleware_receive() -> MutableMapping[str, Any] | dict:
                    ...
                    try:
                        ...
                        provided = cast(Optional[str], form_data.get("csrf_token"))
                        if provided and provided == token:
                            is_valid = True
                    except Exception:
                    ...
                    async def app_receive() -> MutableMapping[str, Any] | dict:
                        if messages_for_app:
                ...
                if not is_valid:
                    response = JSONResponse(
                        {"detail": "CSRF token missing or incorrect."}, status_code=403
                    )
