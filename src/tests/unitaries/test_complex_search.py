...
async def test_complex_search():
    ...
    async with httpx.AsyncClient() as client:
        ...
        payload = {"name": "chinese", "page": 1, "page_size": 10}
        response = await client.post(
            f"{BASE_URL}/api/business-entities-search-by/search", json=payload
        )
        ...
        res_id = await client.post(
            f"{BASE_URL}/api/business-entities-search-by/search",
            json={"name": "chinese-restaurant"},
        )
        if res_id.status_code == 200 and res_id.json()["data"]:
            ...
            payload = {"parent_id": parent_id, "page": 1, "page_size": 10}
            response = await client.post(
                f"{BASE_URL}/api/business-entities-search-by/search", json=payload
            )
            if response.status_code == 200:
        ...
        payload = {"group_name": "admin", "page": 1, "page_size": 10}
        response = await client.post(
            f"{BASE_URL}/api/business-entities-search-by/search", json=payload
        )
        if response.status_code == 200:
