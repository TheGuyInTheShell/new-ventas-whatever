...
class ComparisonValuesController(Controller):
    ...
    async def get_comparisons(
        ...
        comparisons, total = await self.ComparisonValuesService.get_comparisons_paginated(
            db, 
            page=1, 
            page_size=1000, 
            context=context
    ...
    async def create_comparison(self, payload: RQComparisonValue, db: AsyncSession = Depends(get_async_db)):
    ...
    async def update_comparison(self, id: str, payload: RQComparisonValue, db: AsyncSession = Depends(get_async_db)):
    ...
    async def delete_comparison(self, id: str, db: AsyncSession = Depends(get_async_db)):
