...
from src.modules.transactions_buffer.schemas import RQTransactionBuffer, RSTransactionBuffer, RSTransactionBufferList
from src.modules.transactions_buffer.services import create_transaction_buffer


class TransactionBufferController(Controller):
    """
    Controller for Transaction Buffer management.
    
    Path: /api/v1/transactions_buffer
    """

    @Get("/id/{id}", response_model=RSTransactionBuffer, status_code=200)
    ...
    @Get("/", response_model=RSTransactionBufferList, status_code=200)
    ...
    @Post("/", response_model=RSTransactionBuffer, status_code=201)
    async def create_TransactionBuffer_endpoint(
        self, data: RQTransactionBuffer, db: AsyncSession = Depends(get_async_db)
    ) -> RSTransactionBuffer:
        try:
    ...
    @Put("/id/{id}", response_model=RSTransactionBuffer, status_code=200)
    async def update_TransactionBuffer(
        self, id: str, data: RQTransactionBuffer, db: AsyncSession = Depends(get_async_db)
    ) -> RSTransactionBuffer:
        try:
