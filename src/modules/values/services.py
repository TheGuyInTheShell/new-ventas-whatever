...
class ValuesService(Service):
    async def create_value_with_meta(
        ...
        stmt = select(Value).options(selectinload(Value.meta)).where(Value.id == value.id)
    ...
    async def create_values_bulk(
        ...
        for value_data in values_data:
            value = await self.create_value_with_meta(db, value_data)
    ...
    async def get_values_paginated(
        ...
        values = await Value.find_some(
            db, pag=page, order_by=order_by, ord=order, status="exists", filters=filters or {}
        )
