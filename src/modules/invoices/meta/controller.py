...
from .schemas import RQMetaInvoice, RSMetaInvoice, RSMetaInvoiceList

# prefix /invoices/meta
router = APIRouter()

tag = "meta"


@router.get("/id/{id}", response_model=RSMetaInvoice, status_code=200, tags=[tag])
async def get_meta(
...
@router.get("/", response_model=RSMetaInvoiceList, status_code=200, tags=[tag])
async def get_metas(
    pag: Optional[int] = 1,
...
@router.post("/", response_model=RSMetaInvoice, status_code=201, tags=[tag])
async def create_meta(
    meta: RQMetaInvoice, db: AsyncSession = Depends(get_async_db)
) -> RSMetaInvoice:
    try:
...
@router.put("/id/{id}", response_model=RSMetaInvoice, status_code=200, tags=[tag])
async def update_meta(
    id: str | int, meta: RQMetaInvoice, db: AsyncSession = Depends(get_async_db)
) -> RSMetaInvoice:
    try:
