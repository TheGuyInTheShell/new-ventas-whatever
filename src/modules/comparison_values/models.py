from sqlalchemy import ForeignKey, Float, Integer, Index, UniqueConstraint, String
...
class ComparisonValue(BaseAsync):
    ...
    source_value: Mapped["Value"] = relationship(
        "Value",
        foreign_keys=[value_from],
        back_populates="comparisons_from"
    )
    target_value: Mapped["Value"] = relationship(
        "Value",
        foreign_keys=[value_to],
        back_populates="comparisons_to"
    )

    meta: Mapped[list["MetaComparisonValue"]] = relationship("MetaComparisonValue", back_populates="comparison_value")
    ...
    __table_args__ = (
        ...
        UniqueConstraint("value_from", "value_to", "context", name="uix_comparison_values_value_from_value_to")
    )