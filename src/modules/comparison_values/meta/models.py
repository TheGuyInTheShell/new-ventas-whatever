...
class MetaComparisonValue(RelationBaseAsync):
    __tablename__ = "meta_comparison_values"
    key: Mapped[str] = mapped_column(String(100), primary_key=True, nullable=False)
    value: Mapped[str] = mapped_column(Text, nullable=False)
    ref_comparison_value: Mapped[int] = mapped_column(Integer, ForeignKey("comparation_values.id"), nullable=False, primary_key=True)
