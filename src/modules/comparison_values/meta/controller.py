...
@router.get("/id/{id}", response_model=RSMetaComparisonValue, status_code=200, tags=[tag])
async def get_meta(
...
@router.get("/", response_model=RSMetaComparisonValueList, status_code=200, tags=[tag])
async def get_metas(
    pag: Optional[int] = 1,
...
@router.post("/", response_model=RSMetaComparisonValue, status_code=201, tags=[tag])
async def create_meta(
    meta: RQMetaComparisonValue, db: AsyncSession = Depends(get_async_db)
) -> RSMetaComparisonValue:
    try:
...
@router.put("/id/{id}", response_model=RSMetaComparisonValue, status_code=200, tags=[tag])
async def update_meta(
    id: str | int, meta: RQMetaComparisonValue, db: AsyncSession = Depends(get_async_db)
) -> RSMetaComparisonValue:
    try:
