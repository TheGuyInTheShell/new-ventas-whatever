    ...
    try:
        ...
        for tx_data in data.transactions:
            ...
            if not balance_from_id:
                balance_from_id = await _resolve_balance(db, tx_data.ref_value_from, data.business_entity_id)
            if not balance_to_id:
                balance_to_id = await _resolve_balance(db, tx_data.ref_value_to, data.business_entity_id)
...
async def _resolve_balance(db: AsyncSession, value_id: int, entity_id: int) -> int:
...
async def adjust_transaction_and_invoice_service(db: AsyncSession, data: RQAdjustBalance) -> dict:
    """
