...
from ..schemas.values_with_comparison import RQValueWithComparison, RSValueWithComparison, QueryValuesWithComparison, ResultValueWithComparison
...
class DValueWithComparisonService(Service):
    ...
    async def save_value_with_comparison_service(self, db: AsyncSession, data: RQValueWithComparison) -> RSValueWithComparison:
        """
        ...
        """
        try:
            saved_value = await self.ValuesService.create_value_with_meta(db, data.value)
            
            if data.comparison_value.value_from is None:
                data.comparison_value.value_from = saved_value.id
                
            saved_comparison = await self.ComparisonValuesService.create_comparison(db, data.comparison_value)
    ...
    async def update_value_with_comparison_service(self, db: AsyncSession, id: str, data: RQValueWithComparison) -> RSValueWithComparison:
        """
        ...
        try:
            ...
            updated_value = await self.ValuesService.update_value_with_meta(db, id, data.value)
            ...
            if existing_comparison:
                updated_comparison = await self.ComparisonValuesService.update_comparison(db, existing_comparison.uid, data.comparison_value)
            else:
                if data.comparison_value.value_from is None:
                    data.comparison_value.value_from = updated_value.id
                updated_comparison = await self.ComparisonValuesService.create_comparison(db, data.comparison_value)
    ...
    async def get_values_with_comparison_service(self, db: AsyncSession, data: QueryValuesWithComparison) -> ResultValueWithComparison:
        """
