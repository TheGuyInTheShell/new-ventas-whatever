...
class PermissionsService(Service):
    ...
    async def _sync_permission_metadata(
        ...
        meta_query = await db.execute(
            select(MetaPermissions).where(
                MetaPermissions.ref_permission == permission_id
            )
        )
        ...
        for key, value in meta_dict.items():
            ...
            if key in existing_metas:
                ...
                new_meta = MetaPermissions(
                    key=key, value=str_value, ref_permission=permission_id
                )
                db.add(new_meta)
    ...
    def _generate_permissions_hash(self, routes: List[APIRoute], type_name: str) -> str:
        ...
        hash_payload = {"type": type_name, "routes": route_data}
    ...
    async def create_permissions_api(
        ...
        try:
            ...
            if not api_routes:
                print(
                    f"[Permissions] No APIRoute instances found for type '{type}', skipping."
                )
                return
            ...
            if new_permissions:
                print(
                    f"[Permissions] Creating {len(new_permissions)} new permissions..."
                )
            ...
            if owner_role:
                ...
                for p_id in all_permission_ids:
                    if p_id not in existing_link_ids:
                        new_links.append(
                            RolePermission(role_id=owner_role.id, permission_id=p_id)
                        )

                if new_links:
                    print(
                        f"[Permissions] Linking {len(new_links)} new permissions to owner role..."
                    )
    ...
    async def _process_shield_permissions(
        ...
        try:
            ...
            if option_record and option_record.value == current_hash:
                print(
                    f"[Shield Sync] No changes detected (Hash: {current_hash[:8]}...). Skipping sync."
                )
                return

            print(
                f"[Shield Sync] Changes detected. Syncing Shield permissions to DB..."
            )
            ...
            if new_permissions:
                print(
                    f"[Shield Sync] Creating {len(new_permissions)} new permissions..."
                )
            ...
            for role_name in roles_to_sync:
                role_query = await db.execute(
                    select(Role).where(
                        (func.lower(Role.name) == "owner") | (Role.level == 1)
                    )
                )
                ...
                if role:
                    ...
                    for p_id in all_permission_ids:
                        if p_id not in existing_link_ids:
                            new_links.append(
                                RolePermission(role_id=role.id, permission_id=p_id)
                            )

                    if new_links:
                        print(
                            f"[Shield Sync] Linking {len(new_links)} new permissions to {role_name} role..."
                        )
                        db.add_all(new_links)
            ...
            if option_record:
                ...
                db.add(
                    Options(context=context_name, name=option_name, value=current_hash)
                )
